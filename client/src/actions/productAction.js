...
export const getProductBy3to10 =
  (keyword = "", currentPage = 1, category, ratings = 0) =>
  async (dispatch) => {
    try {
      dispatch({ type: ALL_PRODUCT_REQUEST });

      let link = `/api/v1/products?keyword=${keyword}&price[gte]=0&price[lte]=15000000&page=${currentPage}&ratings[gte]=${ratings}`;

      if (category) {
        link = `/api/v1/products?keyword=${keyword}price[gte]=0&price[lte]=15000000&page=${currentPage}&category=${category}&ratings[gte]=${ratings}`;
      }
